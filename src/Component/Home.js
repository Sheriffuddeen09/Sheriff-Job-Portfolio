  ...
  const handleTemplate = (id) => {
    ...
    return (
        ...
        <div>
            ...
            <input type="text" placeholder="job title, keywords, or company" className="w-96 h-12 py-3 px-8 border shadow-slate-700 outline-none border-gray-500 rounded-tl-lg rounded-tr-lg input sm:border-y-0 sm:border-l-0 sm:rounded-tr-none rounded-tl-lg " 
            onChange={handleChange}
            id="search"
            />

            <input type="text" placeholder="City state, zip code, or remote" className="w-96 h-12 border rounded-bl-lg border-gray-500 border-t-0 py-3 px-8 rounded-br-lg 
            sm:border-none relative outline-none"
            onChange={handleChange}
            id="search"
            />
    
            <button className="w-96 h-9 mb-5 bg-blue-700 h-5 font-bold text-white find rounded mt-4 sm:w-32 sm:text-sm bg-blue-700 sm:relative top-1 bottom-4 right-4 sm:translate-x-3" onClick={handleSubmit}>Find Jobs</button>
            </form>
        </div>
        <Accordion />
        <div>
