...
const JobApplySecond = ({ApplyDeleteSecond, jobseconds}) =>{
    ...
    const content = (
        <div>
            {
                jobseconds.map((apply) =>{
                   return ( 
                   <>
                   <div className="flex flex-row justify-around flex-wrap p-2 mt-10">
...
<button className="w-48 mb-10 mt-6 text-white p-2 font-bold rounded-lg bg-red-700" onClick={ApplyDeleteSecond}>Withdraw application</button>
                    </div>
                    </div>
                    </>
                )})
