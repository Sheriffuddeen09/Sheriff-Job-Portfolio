...
const EditNewLanguage= ({newlanguage, newlanguages, setNewLanuages}) =>{

    const navigate = useNavigate()
    const handleUpdate = (e) =>{
        const newlist = newlanguages.map((li) =>(
            li.id === newlanguage.id ? {...li, [e.target.name] : e.target.value} : li
        ))
        setNewLanuages(newlist)
    }
...
export default EditNewLanguage