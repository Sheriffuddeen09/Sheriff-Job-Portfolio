...
const LanguageId = ({languages, newlanguages}) =>{
    ...
    const {id} = useParams()
    const language = languages.find((language) => (language.id).toString() === id) 
        ...
        <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 15.75 3 12m0 0 3.75-3.75M3 12h18" />
                            ...
                            <div className="">
                                    <p className="font-bold text-3xl mb-6 mt-3">Language</p>

                                <hr className="my-4"/>
                                <div className="flex flex-row justify-between gap-44 flex-wrap lg:gap-96">
                        <span>
                        <p className="mb-2">{language.title}</p>
                        <p className="mb-2">{language.name}</p>
                        </span>
                        <Link to={`/lan/${language.id}`}>   
                 <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"  stroke-width="1.5" stroke="currentColor" class="size-6 translate-y-3 cursor-pointer">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                        ...
                        <hr className="my-4"/>
                                   <NewLanguage newlanguages={newlanguages}/>
                                   <hr className="my-4"/>
                                <div className="flex flex-row justify-between flex-wrap lg:gap-60">
                                    <span>
                                    <p className="font-bold mb-2 text-blue-700">Add Another Certification</p>
                                    </span>
                                    <Link to={`/langs/${language.id}`}>   
